...
class Crypt
{
	...
	public function encrypt($str){
		...
		return $this->createCombine($iv, $hmac, $cipherText);
	...
	public function decrypt($str){
		...
		$original_plaintext = openssl_decrypt($crypt_data['str'], $this->cryptMethod, CRYPT_KEY, OPENSSL_RAW_DATA,
			$crypt_data['iv']);
	...
	protected function createCombine($str, $iv, $hmac){
		$new_str = '';

		$str_len = strlen($str);

		$counter = (int)ceil(strlen(CRYPT_KEY) / ($str_len + $this->hacheLength));
