	...
	protected function createData($arr = [])
	{
		...
		if ($arr['fields']) {
			if (is_array($arr['fields'])) {
				$fields = Settings::instance()->arrayMergeReqursive($fields, $arr['fields']);
			} else {
				$fields[] = $arr['fields'];
			}
		}
		...
		if ($arr['order']) {
			if (is_array($arr['order'])) {
				$order = Settings::instance()->arrayMergeReqursive($order, $arr['order']);
			} else {
				$order = $order['order'];
			}
		}
		if ($arr['order_direction']) {
			if (is_array($arr['order_direction'])) {
				$order_direction = Settings::instance()->arrayMergeReqursive($order_direction, $arr['order_direction']);
			} else {
