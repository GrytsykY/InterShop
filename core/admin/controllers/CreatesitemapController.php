...
class CreatesitemapController extends BaseAdmin
{
	...
	public function inputData($links_counter = 1, $redirect = true){
		...
		// проверяем есть ли что нибудь в БД
		...
		while ($this->temp_links){
			...
			if ($temp_links_count > $this->maxLinks){
				...
				for ($i = 0; $i < $count_chunks; $i++){
					...
					// разрегистрируем $links[$i]
					unset($links[$i]);

					if ($links){

						foreach ($table_rows as $key => $item){
	...
	protected function parsing($urls){
		...
		do{
			...
			if (false !== $info){
				if ($info['result'] !== 0){
					$i = array_search($info['heandle'], $curl);
		...
		}while($status === CURLM_CALL_MULTI_PERFORM || $active); //$status === CURLM_CALL_MULTI_PERFORM ||
		...
		foreach ($urls as $i => $url){
			...
			if (!preg_match('/Content-Type:\s+text\/html/ui',$result[$i])) {
				...
				$this->cancel(0,'Incorect content type ',$url);
			...
			if (!preg_match('/HTTP\/\d\.?\d?\s+20\d/ui',$result[$i])){
				...
				$this->cancel(0,'Incorect server code ',$url);
		...
		if ($content){
			...
			if ($links[2]){
				foreach ($links[2] as $link) {
					...
					$site_url = str_replace('.','\.',str_replace('/','\/',SITE_URL));
